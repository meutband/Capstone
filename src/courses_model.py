...
#Upload the data
data = pd.read_excel('data/courses_data.xlsx')
data['Intercept'] = 1
...
#Make an interaction between courses and adoption percent
...
#backwards stepwise selection individual step
...
def backwards2(p, train, test):
    ...
    if p != 1.:
        ...
        #If a interaction is in the results but the components arent then the components are put into results
...
#Run kfold for backwards selection p value. For each kfold operation, use train, test
#for backwards stepwise selection
def kfold(p):
    error = np.empty(10)
    index = 0
    df = pd.DataFrame()
...
#Kfolds cross validation with num folds = 10
